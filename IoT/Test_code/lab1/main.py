...
class ClientSocket:
    ...
    def connectServer(self):
        ...
        try:
            ...
            if self.connectCount == 10:
                print_log(u'Connect fail %d times. exit program' %
                          (self.connectCount))
                # sys.exit()
            else:
                print_log(u'%d times try to connect with server' %
                          (self.connectCount))
                self.connectServer()
    ...
    def recv(self):
        ...
        while True:
            ...
            if (recv_data == key_up and flag_up == False):
                flag_up = True
            if (recv_data == key_down and flag_down == False):
                flag_down = True
            if (recv_data == key_shift and flag_shift == False):
                flag_shift = True
            if (recv_data == key_left and flag_left == False):
                flag_left = True
            if (recv_data == key_right and flag_right == False):
                flag_right = True
            if (recv_data == key_release and flag_release == False):
                flag_release = True
            if (recv_data == start_signal and flag_start == False):
                flag_start = True
                send_racing_data_thread = threading.Thread(
                    target=send_racing_data)
...
class MyApp(QMainWindow, Ui_MainWindow):
    ...
    def readySignal(self):
        ...
        car_status = 1
        temp_data = make_json(car_no, cur_time, car_speed,
                              car_gate, car_status)
...
def send_racing_data():
    ...
    try:
        ...
        while flag_start:
            ...
            car_status = 3
            temp_data = make_json(
                car_no, cur_time, car_speed, car_gate, car_status)
            client.sendData(temp_data)
            time.sleep(0.1)
...
if __name__ == "__main__":
    ...
    global flag_up, flag_down, flag_left, flag_right, flag_shift, flag_ctrl, flag_release, flag_start
    global flag_motor_connect
